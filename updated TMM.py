...
# ==========================================
    ...
    # 🚨 THE NEW UNBLOCKABLE SYSTEM PROMPT 🚨
    # Instructs Llama 3.3 to act as an SVG graphics engine.
    ...
    CRITICAL INSTRUCTION FOR DIAGRAMS AND VISUALS:
    You are a master at coding pure SVG (Scalable Vector Graphics) to illustrate educational concepts.
    If a student asks for a diagram, image, or visual representation of ANY subject (Physics, Biology, Chemistry), YOU MUST DRAW IT YOURSELF USING PURE SVG CODE.
    
    SVG Rules:
    1. Use <svg viewBox="0 0 800 400" xmlns="http://www.w3.org/2000/svg">
    2. ALWAYS include a dark background: <rect width="100%" height="100%" fill="#1e293b"/>
    3. Draw precise geometric shapes, lines, arrows, and text for the diagrams. 
    4. Make the text highly readable with fill="white" or bright colors.
    5. Be highly descriptive with labels.
    
    DO NOT provide external image URLs. NEVER use markdown image links like ![alt](url). ONLY generate pure SVG code.
    """
    ...
    # --- 🚨 THE NEW LOCAL VECTOR PARSER 🚨 ---
    # This intercepts the SVG math generated by Llama 3 and paints it directly to your screen.
    def render_chat_content(text):
        if not text: 
            return
            
        # Nuke any hallucinated markdown links so they don't show up as broken icons
        text = re.sub(r'!\[([^\]]*)\](?:\([^\)]*\))?', '', text)
        
        # Split the text strictly by looking for the actual SVG code blocks
        parts = re.split(r'(<svg.*?</svg>)', text, flags=re.IGNORECASE | re.DOTALL)
        
        for part in parts:
            if part.strip().lower().startswith('<svg'):
                # We caught an SVG graphic drawn by the AI!
                # We clean it up and render it as a local Base64 image so nothing can block it.
                clean_svg = part.replace('```xml', '').replace('```', '').strip()
                try:
                    b64 = base64.b64encode(clean_svg.encode('utf-8')).decode('utf-8')
                    img_html = f'''
                    <div style="margin: 15px 0; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.5); background: #1e293b;">
                        <img src="data:image/svg+xml;base64,{b64}" style="width: 100%; display: block;">
                    </div>
                    <p style="color: #aaa; font-size: 13px; font-style: italic; text-align: center;">🎨 AyA Local Vector Rendering</p>
                    '''
                    st.markdown(img_html, unsafe_allow_html=True)
                except Exception:
                    st.error("⚠️ Failed to render local vector diagram.")
            else:
                # Normal text processing
                clean_text = part.replace('```xml', '').replace('```', '')
                if clean_text.strip():
    ...
    if st.session_state.aya_messages and st.session_state.aya_messages[-1]["role"] == "user":
        with st.chat_message("assistant"):
            with st.spinner("🤖 AyA is thinking and generating vectors..."):
                try:
