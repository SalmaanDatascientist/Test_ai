...
# ==========================================
    ...
    # --- 🚨 THE FINAL, UNBLOCKED PARSER 🚨 ---
    def render_chat_content(text):
        if not text: 
            return
            
        # 1. Catch ALL the broken formatting the AI hallucinates (Fixes Screenshots 1 & 2)
        text = re.sub(r'!\[([^\]]+)\](?:\([^\)]*\))?', r'[IMAGE: \1]', text)
        text = re.sub(r'\{IMAGE:\s*(.*?)\}', r'[IMAGE: \1]', text, flags=re.IGNORECASE)
        text = re.sub(r'\[\[IMAGE:\s*(.*?)\]\]', r'[IMAGE: \1]', text, flags=re.IGNORECASE)
        
        # 2. Split the text by our specific tags
        ...
        for i, part in enumerate(parts):
            if i % 2 == 0:
                ...
                if prompt:
                    # Clean the prompt of weird characters so it doesn't break the URL
                    clean_prompt = re.sub(r'[^a-zA-Z0-9\s\-,]', '', prompt)[:150].strip()
                    if not clean_prompt:
                        clean_prompt = "educational diagram"
                        
                    safe_prompt = urllib.parse.quote(clean_prompt)
                    seed = random.randint(1, 1000000)
                    
                    # 🚨 THE ROOT CAUSE FIX: I REMOVED the `&nologo=true` command.
                    # Pollinations made nologo a premium feature, which caused the 403 Forbidden errors!
                    url = f"https://pollinations.ai/p/{safe_prompt}?width=800&height=400&seed={seed}"
                    
                    st.markdown(f"🎨 **AyA Visual:** *{clean_prompt}*")
                    
                    # Injecting a direct HTML image tag bypasses Streamlit's proxy blocks
                    img_html = f'''
                    <img src="{url}" alt="{clean_prompt}" 
