...
# ── AyA image renderer — CLIENT-SIDE via st.components ──────────────────────
#
# ROOT CAUSE of the "image not loading" bug on Streamlit Cloud:
#   1. st.markdown("![alt](url)")  → blocked by Streamlit's CSP header
#   2. requests.get(url)           → blocked by Streamlit Cloud's outbound
#                                    network egress firewall
#
# THE FIX: Use st.components.v1.html() to inject a raw <img> tag.
#   The component runs inside its OWN sandboxed iframe which has its own
#   network context — the USER'S BROWSER fetches the image directly from
#   Pollinations, completely bypassing both the CSP and server egress issues.
#
import streamlit.components.v1 as components

def render_image_client_side(prompt: str):
    """
    Renders a Pollinations AI image entirely in the user's browser.
    No server-side network call needed — immune to Streamlit Cloud egress blocks.
    Uses a stable seed derived from the prompt so the same question always
    ...
    url = (
        ...
        f"?width=820&height=420&nologo=true&seed={seed}"
    )

    # The HTML runs in the browser iframe — no Streamlit server involvement
    html = f"""
    <div style="font-family:sans-serif;">
        ...
        />
        <div id="err" style="display:none;border:2px dashed #555;border-radius:8px;
                              padding:20px;text-align:center;color:#aaa;background:#1a1a2e;">
            🖼️ <b>Visual:</b> {prompt}<br>
            <small>Could not load — check your internet connection or try again.</small>
        </div>
    </div>
    """
    # height = image height + small buffer; scrolling=False removes the iframe scrollbar
